...
class NewsListViewController: UIViewController {
    ...
    var viewmodel = NewsListViewModel(serviceProtocol: MostPopularService())
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.title = IdentifierNameConstants.nagivationTitle
        self.addObservers()
        self.setup()
    }
    ...
    func moveDetail(feed: Feed) {
        guard let detailcont = self.storyboard?.instantiateViewController(identifier:
                                    IdentifierNameConstants.detailViewControllerName) as?
                DetailViewController else { return }
        detailcont.detailViewModel = DetailViewModel(newsFeed: feed)
...
extension NewsListViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard  let cell = tableView.dequeueReusableCell(withIdentifier:
                                    IdentifierNameConstants.newsTableViewCellName) as?
                NewsTableViewCell else {
            fatalError(ErrorConstants.cellRegistarationFailed)
        }
